"""Payment runs (7.5 Zahllauf, 6.9.9, D06, D35 to D37).
...
Exporting a file (payment initiation) requires release gate G2. Open items are settled only when
the execution is proven by an imported bank transaction (D06); callbacks repeat without effect.
"""
...
from mhvp.accounting.models import EntrySource, Invoice, OpenItem, PostingStatus
...
async def approve(
    ...
    if order.created_by == user_id and not valid:
        pass  # the creator may be one of the two approvers
    session.add(
        PaymentApproval(
