...
function toForm(row: ServiceContract): FormState {
  return {
    ...
    auto_renewal_months: row.auto_renewal_months === null ? "" : String(row.auto_renewal_months),
...
function toBody(f: FormState) {
  return {
    ...
    auto_renewal_months: f.auto_renewal_months ? Number(f.auto_renewal_months) : null,
...
export function ServiceContracts({
  ...
  async function submit(event: React.FormEvent) {
    ...
    setBusy(true);
    const path = editing ? `/api/bff/service-contracts/${editing}` : "/api/bff/service-contracts";
    ...
    setError(null);
    setRows((list) =>
      editing ? list.map((r) => (r.id === editing ? result.data : r)) : [...list, result.data],
    );
    setEditing(null);
    setForm(EMPTY);
  }
  ...
  async function remove(id: string) {
    ...
    const result = await bff<null>(`/api/bff/service-contracts/${id}`, { method: "DELETE" });
    if (result.ok) setRows((list) => list.filter((r) => r.id !== id));
    else setError(result.message ?? t("saveError"));
  }

  const notice = (row: ServiceContract) =>
    `${row.notice_period_days} ${t(`unit.${row.notice_period_unit}`)}`;
  ...
  return (
    <div className={ui.pageGap}>
          ...
          <table className={ui.table} data-testid="service-contracts-table">
            ...
            <tbody>
              {rows.map((row) => (
                <tr key={row.id}>
                  <td>{row.title}</td>
                  <td>{providerName.get(row.provider_contact_id) ?? t("unknown")}</td>
                  <td>{row.property_id ? (propertyName.get(row.property_id) ?? t("unknown")) : ""}</td>
                  <td className="tabular-nums">
                    {formatDate(row.starts_at)} {row.ends_at ? `${t("until")} ${formatDate(row.ends_at)}` : t("openEnded")}
                    {row.auto_renewal_months ? ` (${t("renewal", { count: row.auto_renewal_months })})` : ""}
                  </td>
                  <td className="tabular-nums">{notice(row)}</td>
                  <td className="tabular-nums">{formatDate(row.next_possible_end)}</td>
                  <td className="tabular-nums">
                    {row.latest_notice_date ? (
                      <>
                        {formatDate(row.latest_notice_date)} <span className={ui.badge}>{t("toCheck")}</span>
                      </>
                    ) : null}
                  </td>
                  <td>{t.has(`status.${row.status}`) ? t(`status.${row.status}`) : row.status}</td>
                  <td className="whitespace-nowrap">
                    ...
                    {canDelete ? (
                      <button type="button" className={ui.button} onClick={() => remove(row.id)}>
                        {t("delete")}
                      </button>
                    ) : null}
                  </td>
      ...
      {showForm ? (
        ...
        >
          <h2 id="service-contract-form-title" className={`${ui.h2} md:col-span-2`}>
            {editing ? t("editTitle") : t("createTitle")}
          </h2>
          <label className="flex flex-col gap-1">
            <span className={ui.label}>{t("field.title")}</span>
            <input className={ui.input} required maxLength={300} value={form.title} onChange={(e) => set("title", e.target.value)} />
          </label>
          <label className="flex flex-col gap-1">
          ...
          <label className="flex flex-col gap-1">
            ...
            <select className={ui.input} value={form.property_id} onChange={(e) => set("property_id", e.target.value)}>
          ...
          <label className="flex flex-col gap-1">
            ...
            <input className={ui.input} type="date" required value={form.starts_at} onChange={(e) => set("starts_at", e.target.value)} />
          </label>
          <label className="flex flex-col gap-1">
            <span className={ui.label}>{t("field.endsAt")}</span>
            <input className={ui.input} type="date" value={form.ends_at} onChange={(e) => set("ends_at", e.target.value)} />
          </label>
          <div className="flex gap-2">
            ...
            <label className="flex flex-col gap-1">
              ...
              <select
                ...
                onChange={(e) => set("notice_period_unit", e.target.value as "days" | "months")}
              >
          ...
          <label className="flex flex-col gap-1">
            ...
            <input className={ui.input} type="date" value={form.cancelled_at} onChange={(e) => set("cancelled_at", e.target.value)} />
          </label>
          <label className="flex flex-col gap-1 md:col-span-2">
            <span className={ui.label}>{t("field.notes")}</span>
            <textarea className={ui.input} maxLength={5000} value={form.notes} onChange={(e) => set("notes", e.target.value)} />
          </label>
